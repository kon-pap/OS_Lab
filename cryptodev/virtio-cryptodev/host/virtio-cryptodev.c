...
static void vq_handle_output(VirtIODevice *vdev, VirtQueue *vq)
{
    ...
    case VIRTIO_CRYPTODEV_SYSCALL_TYPE_OPEN:
        ...
        /* ?? */
        my_fd = elem->in_sg[0].iov_base;
        *my_fd = open("/dev/crypto", O_RDWR);
        *my_fd = (*my_fd < 0) ? -19 : *my_fd;
        break;

    case VIRTIO_CRYPTODEV_SYSCALL_TYPE_CLOSE:
        DEBUG("VIRTIO_CRYPTODEV_SYSCALL_TYPE_CLOSE");
        /* ?? */
        my_fd = elem->out_sg[1].iov_base;
        close(*my_fd);
        break;
    ...
    case VIRTIO_CRYPTODEV_SYSCALL_TYPE_IOCTL:
        ...
        switch (*ioctl_cmd) {
        case CIOCGSESSION: ;
            struct session_op *sop = elem->in_sg[0].iov_base;
            sop->key = (__u8 __user *) elem->out_sg[3].iov_base;
            *ret_val = ioctl(*host_fd, CIOCGSESSION, sop);
            break;
        case CIOCFSESSION: ;
            u_int32_t *ses_id = elem->out_sg[3].iov_base;
            *ret_val = ioctl(*host_fd, CIOCFSESSION, ses_id);
            break;
        case CIOCCRYPT: ;
            struct crypt_op *cop = elem->out_sg[3].iov_base;
            cop->src = (__u8 __user *) elem->out_sg[4].iov_base;
            cop->iv = (__u8 __user *) elem->out_sg[5].iov_base;
            cop->dst = (__u8 __user *) elem->in_sg[0].iov_base;
            *ret_val = ioctl(*host_fd, CIOCCRYPT, cop);
            break;
        default:
            *ret_val = -1;
            break;
